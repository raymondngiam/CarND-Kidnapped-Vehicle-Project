...
/* Prints observations.
 * @param observations A vector of LandmarkObs which can be multiple sensor observations.
 */
inline void printObservations(const std::vector<LandmarkObs>& observations){
	for (unsigned int i=0; i<observations.size(); ++i){
		const LandmarkObs& obs = observations[i];
		std::cout<<i<<": "<<obs.id<<","<<obs.x<<","<<obs.y<<std::endl;
	} 
}
