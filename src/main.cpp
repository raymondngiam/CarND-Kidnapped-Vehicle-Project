    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
		  ...
		  // Debug: output raw sensor observation in vehicle coord system
		  cout<<"Observations"<<endl;
		  printObservations(noisy_observations);
		  ...
		  // Debug: output transformed sensor observations in map coord
		  vector<LandmarkObs> observations_map_coord;
		  for (const auto& obs : noisy_observations){
		      LandmarkObs l = pf.transVehicleFrameToMapFrame(best_particle,obs);
			  observations_map_coord.emplace_back(l);
		  }
		  cout<<"Observations Map Coord"<<endl;
		  printObservations(observations_map_coord);

		  // Debug: output data association for transformed sensor observations w.r.t. map landmarks
		  vector<LandmarkObs> landmarks_map_coord;
		  landmarks_map_coord.reserve(map.landmark_list.size());
		  ...
		  pf.dataAssociation(landmarks_map_coord,observations_map_coord,sensor_range);
		  ...
		  cout<<pf.getAssociations(best_particle)<<endl<<endl;
